...
**圆角与半透明**
窗口用透明背景实现圆角，**不使用外投影**（投影会被窗口边缘裁成硬边灰环），四周保持完全透明。**部分 Windows 机器上，硬件加速与 DWM 合成冲突会让透明窗口渲染出黑底/黑框**（圆角卡片外那圈黑色矩形，见 electron/electron#40515）；另有机器相反，关掉硬件加速会变白底、甚至窗口不可见。

程序**默认不关硬件加速**（保持系统原生渲染）。若你的机器上透明窗口出现**黑框**，可在设置面板打开「**兼容模式**」（关闭硬件加速），或用 `--no-gpu` 启动试试：
- ⚠️ 注意：个别机器上关掉硬件加速反而会变成**白底、甚至窗口完全不可见**——若遇到请立刻关掉「兼容模式」（或用 `--gpu` 启动恢复）。
- 启动参数强制：`--no-gpu` 关加速、`--gpu` 开加速（均覆盖设置）。
- 若环境关闭了桌面合成（DWM），圆角会退化成直角，此时用 `--no-transparency` 启动，或在设置面板里关掉 `transparency`。
